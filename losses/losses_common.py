...
class ECharbonier():
    ...
    def interpolate_two_masks(self, mask0, mask1, alpha=0.5):
        return (1 - alpha) * mask0 + alpha * mask1
    ...
    def mask(self,e):
        ...
        for i in range(n_segments):
            ...
            seg = e[:, start:end, :, :]   # [1, 25, H, W]
            mask = seg.abs().sum(dim=1, keepdim=True)  # [1, 1, H, W]
           
            base_masks.append(mask)
        
        interp_list = []
        for i in range(len(base_masks) - 1):
            interp = self.interpolate_two_masks(base_masks[i], base_masks[i+1])
            if self.emask_type == 'flag':
                interp = interp/torch.max(interp, torch.tensor(1e-5).to(e.device))
            interp_list.append(interp)
        e_mask = torch.cat(interp_list, dim=0).unsqueeze(0)
        return e_mask
